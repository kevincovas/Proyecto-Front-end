...
export default class LigaItaliana extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      ligaItaliana: [],
    ...
    return (
      <>
        ...
        <table style={tableStyle}>
          ...
          {this.state.ligaItaliana.map((scorer) => (
          <tr style={tableStyle}>
  ...
  async componentDidMount() {
    ...
    const ligaItaliana = await axios.get(
      "https://api.football-data.org/v2/competitions/SA/scorers",
      { headers }
    );

    //Console logs
    console.log(ligaItaliana.data);
    this.setState({ ligaItaliana: ligaItaliana.data.scorers });
    const info = this.state.ligaItaliana;
    //console.log(info);
  }
}
const tableStyle = {
